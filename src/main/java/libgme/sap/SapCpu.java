 ...
 * <p>
 * Same core as {@link libgme.nsf.NesCpu} but uses flat 64K memory without
 * banking, and stops at a BRK at {@link #idleAddr} which marks return from
 * the player routine.
 *
 * @see "https://www.slack.net/~ant"
 */
public final class SapCpu {

    public static final int idleAddr = 0xFEFF;

    // Resets registers and keeps pointer to 64K memory passed in
    ...
    }; // 0x00 was 7; the real BRK path adds its 7 clocks itself so that the idle BRK is free
    ...
    public void runCpu() {
        ...
        int p = this.p;
        int c = this.c;
        int[] instrTimes = SapCpu.instrTimes;
...
loop:
        while (time < 0) {
            ...
            int opcode;
            this.time =
                    (time += instrTimes[opcode = 0xff & mem[pc]]);
            instr = pc + 1;
            ...
            switch (opcode) {
                ...
                // Illegal
                default:
                    // stop with pc left at the illegal instruction; caller
                    // reports an emulation error when pc != idleAddr
                    break loop;
                ...
                case 0x00: // BRK
                    if (pc >= idleAddr) {
                        // routine returned; leave pc at idle address and stop
                        break loop;
                    }
                case 0x08: // PHP
                    break;
            ...
            switch (opcode) {
                ...
                case 0x00: {// BRK
                    this.time = (time += 7); // clocks are added here rather than by the table
                    int t = pc + 2;
            ...
            switch (opcode) {
                ...
                case 0x00: {// BRK
                    ...
                    if (opcode == 0x00) // interrupt entered: set I, clear D
                        p = (p & ~D08) | I04;
                    continue;
                }
